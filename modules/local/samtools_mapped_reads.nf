process SAMTOOLS_MAPPED_READS {
    ...
    if ! samtools view -H ${bam_files} > /dev/null; then
    echo "Error: BAM file A_PB2.bam has no headers. Skipping this file."
    # set number_mapped_reads and mean_depth to 0
    echo -e "${prefix}\t\${segment_name}\t0\t0" > ${prefix}_\${segment_name}.bam_results.tsv
    exit 0
    fi
