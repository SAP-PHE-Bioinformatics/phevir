...
def collapse_depth_columns(df):
    # Identify HA and NA columns for MeanDepth and Coverage
    ha_depth_columns = [col for col in df.columns if col.startswith("Average_Depth_HA_")]
    na_depth_columns = [col for col in df.columns if col.startswith("Average_Depth_NA_")]
    ha_coverage_columns = [col for col in df.columns if col.startswith("Coverage_HA_")]
    na_coverage_columns = [col for col in df.columns if col.startswith("Coverage_NA_")]

    # Collapse HA and NA depth columns
    if ha_depth_columns:
        df['Average_Depth_HA'] = df[ha_depth_columns].mean(axis=1)
        # df = df.rename(columns={'Depth_HA': 'Average_Depth_HA'})

    if na_depth_columns:
        df['Average_Depth_NA'] = df[na_depth_columns].mean(axis=1)
        # df = df.rename(columns={'Depth_NA': 'Average_Depth_NA'})

    # Collapse HA and NA Coverage columns
    if ha_coverage_columns:
        df['Coverage_HA'] = df[ha_coverage_columns].mean(axis=1)
#        df = df.rename(columns={'Coverage_HA': 'Coverage_HA'})
    if na_coverage_columns:
        df['Coverage_NA'] = df[na_coverage_columns].mean(axis=1)
#        df = df.rename(columns={'Coverage_NA': 'Coverage_NA'})
...
# Function to ensure three parts in column names
# def ensure_three_parts(col):
#     if isinstance(col, tuple):
#         parts = [str(c) for c in col if c]  # Remove empty/None values
        
#         if len(parts) < 3:
#             missing_count = 3 - len(parts)
#             parts = (['Unknown'] * missing_count) + parts  # Prepend placeholders
        
#         return '_'.join(parts)
#     return col  # Leave non-tuple column names unchanged
...
def main(tsv_file_path, excel_file_path, qc_file_path, kraken2_file_path, drug_sensitivity_path, typing_report_path, irma_qc_path, run_id, mut):
    ...
    df_qc_cleaned = df_qc.dropna(subset=['Sample'])
    df_qc_cleaned = df_qc_cleaned.groupby(['Sample', 'Segment'], as_index=False).agg({'MeanDepth': 'mean', 'Coverage': 'mean'})
   # df_qc_pivot = df_qc_cleaned.pivot(index='Sample', columns='Segment', values=['MeanDepth', 'Coverage']).reset_index()
    df_qc_pivot = df_qc_cleaned.pivot(index='Sample', columns='Segment', values=['MeanDepth', 'Coverage']).fillna('-').reset_index()
    df_qc_pivot = df_qc_pivot.rename(columns=lambda x: f"Depth_{x}" if x != 'Sample' else x)
    # Pivot MeanDepth and Coverage columns
    df_qc_pivot = df_qc_cleaned.pivot(index='Sample', columns='Segment', values=['MeanDepth', 'Coverage']).reset_index()
  ...
  #  df_qc_pivot.columns = [ensure_three_parts(col) for col in df_qc_pivot.columns]
    print(df_qc_pivot.columns)  # Check columns after collapsing depth columns 
    # Rename columns to have consistent names
    df_qc_pivot = df_qc_pivot.rename(columns=lambda x: x.replace('MeanDepth_', 'Average_Depth_').replace('Coverage_', 'Coverage_'))
    ...
    df_excel['Type'] = df_excel['H: top match virus name'].apply(
        lambda x: 'Influenza A virus' if pd.notnull(x) and ('Influenza A' in x or 'Alphainfluenzavirus' in x) else (
            'Influenza B virus' if pd.notnull(x) and 'Influenza B' in x else 'Other'
        )
    )
    df_excel.rename(columns={'Subtype Prediction': 'blastn_Subtype'}, inplace=True)
    ...
    df_merged = pd.merge(df_merged, df_kraken2, on='Sample', how='outer')
    ...
    df_irma.rename(columns={'Subtype Prediction': 'abricate_Subtype'}, inplace=True)
    ...
    df_merged['Genomic_QC'] = df_merged.apply(
        lambda row: 'PASS' if row.get('Average_Depth_HA', 0) > 30 and row.get('Average_Depth_NA', 0) > 30 else 'FAIL', axis=1
    )


    # Reorder columns
    desired_order = ['Sample', 'RunID', 'RunDate'] + \
    [col for col in df_merged.columns if 'Depth_' in col or 'Coverage_' in col] + \
    ['Genomic_QC'] + \
    ...
    # Export to CSV
    output_csv_path = f'{run_id}_detailed_IAV.csv'
    df_merged.to_csv(output_csv_path, index=False)
    
    
# Create a short summary DataFrame for PHEED
    df_pheed = df_merged[[
        'Sample', 'RunID', 'RunDate', 'Average_Depth_HA', 'Average_Depth_NA', 
        'Coverage_HA', 'Coverage_NA', 'Genomic_QC', 'IRMA_type', 'IRMA_subtype', 
        'clade', 'short-clade', 'Type', 'baloxavir_sensitivity', 
        'laninamivir_sensitivity', 'oseltamivir_sensitivity', 
        'peramivir_sensitivity', 'zanamivir_sensitivity'
    ]]

# Rename columns for PHEED
    df_pheed.rename(columns={'IRMA_subtype': 'Subtype'}, inplace=True)

# Export the PHEED summary table to CSV
    pheed_csv_path = f'{run_id}_IAV.csv'
    df_pheed.to_csv(pheed_csv_path, index=False)

    print(f"Detailed output saved to: {output_csv_path}")
    print(f"PHEED summary saved to: {pheed_csv_path}")
